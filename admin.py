...
def remove_orders(all_orders:dict, menu:dict) -> dict:
    ...
    selected_item = iterfzf([f"{orders['name']:{max(map(lambda orders: len(orders['name']), all_orders['orders']))}} ({orders['id']})" for orders in all_orders['orders']], 
                            prompt="Select an order to remove: ", 
                            ansi=True
                            )
